...
export async function getProductInsight(
  ...
  if (cached && Date.now() - cached.generatedAt.getTime() < INSIGHT_TTL_MS) {
    return { tip: cached.tip, priceAssessment: cached.priceAssessment };
  }

  if (product.offers.length === 0) return cached ?? null;
  ...
  try {
    ...
    return cached ? { tip: cached.tip, priceAssessment: cached.priceAssessment } : null;
  }
}
